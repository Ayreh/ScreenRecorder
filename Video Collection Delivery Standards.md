...
| Supplier Code | Unique identifier allocated by us for each supplier, used for S3 directory differentiation, permission control, and data traceability | 1. Format: 3-6 letters + numbers combination (e.g., SUP001, SUP002), pre-filed and allocated by us, supplier cannot customize; <br>2. Purpose: First-level S3 directory, web end authentication, log traceability (distinguish data from different suppliers). |